...
const ConfirmDeleteMusic: NextPage = () => {
  const router = useRouter();
  const { id } = router.query;
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState('');
  const [musicData, setMusicData] = useState<{ title: string } | null>(null); // État pour stocker les données de la vidéo
  ...
  const deleteMusic = async () => {
    try {
      ...
      const response = await axios.delete(`/api/dashboard/deleteMusic?id=${id}`);
            ...
            <div className="mt-8">
              {musicData && (
                <>
                  ...
                  <p>Titre de la vidéo: {musicData.title}</p>
                </>
              )}
...
export default ConfirmDeleteMusic;
